...
- Desde la página de resumen del integrante ([[US-08-resumen-por-integrante]]) se puede pasar a un modo "Cargar boleta", separado del flujo de registrar uso.
- El formulario pide: proveedor (selector sobre [[proveedores]] activos — no texto libre, para poder agregar montos por proveedor de forma confiable más adelante), período (selector de mes, mismos últimos 12 meses que [[US-07-resumen-del-mes]] — no texto libre, para no tener variantes del mismo mes escritas distinto) y monto total.
